describe('Select - Events - deselect', function() {
	let table;
	...
	describe('Check the defaults', function() {
		...
		it('Called after deselection', function() {
	...
	describe('Functional tests - api', function() {
		...
		it('cell', function() {
			table.cell(2, 1).select().deselect();
		...
		it('cells', function() {
			table.cells([{ row: 2, column: 2 }, { row: 2, column: 0 }]).select().deselect();
			expect(params[2]).toBe('cell');
			expect(params[3]).toEqual([{ row: 2, column: 2 }, { row: 2, column: 0 }]);
		});
		it('row', function() {
			table.row(3).select().deselect();
			expect(params[2]).toBe('row');
			expect(params[3]).toEqual([3]);
		});
		it('rows', function() {
			table.rows([5, 50]).select().deselect();
			expect(params[2]).toBe('row');
			expect(params[3]).toEqual([5, 50]);
		});
		it('column', function() {
			table.column(3).select().deselect();
